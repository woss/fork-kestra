        ...
        <DataTable @page-changed="onPageChanged" ref="dataTable" :total="total">
            ...
            <template #table>
                ...
                >
                    ...
                    <el-table-column prop="key" sortable="custom" :sortOrders="['ascending', 'descending']"
                    ...
                    <el-table-column columnKey="locked" className="row-action">
                        <template #default="scope">
                            <el-tooltip
                                v-if="scope.row.namespace !== undefined && areNamespaceSecretsReadOnly?.[scope.row.namespace]"
                                transition="" :hideAfter="0" :persistent="false" effect="light"
                            >
                                <template #content>
                    ...
                    <el-table-column columnKey="copy" className="row-action">
                        <template #default="scope">
                            <el-tooltip :content="$t('copy_to_clipboard')">
                                <el-button :icon="ContentCopy" link
                    ...
                    <el-table-column v-if="!keyOnly && !paneView" columnKey="update" className="row-action">
                        <template #default="scope">
                            <el-button v-if="canUpdate(scope.row)" :icon="FileDocumentEdit" link
                    ...
                    <el-table-column v-if="!keyOnly && !paneView" columnKey="delete" className="row-action">
                        <template #default="scope">
                            <el-button v-if="canDelete(scope.row)" :icon="Delete" link
        ...
        >
            <el-form class="ks-horizontal" :model="secret" :rules="rules" ref="form">
                ...
                <el-form-item :label="$t('secret.description')" prop="description">
                    <el-input v-model="secret.description" :placeholder="$t('secret.descriptionPlaceholder')"
                              required
                    />
                </el-form-item>
            ...
            filteredSecrets() {
                return this.namespace === undefined
                    ? this.secrets?.filter((secret: {
                        key: string
                    }) => !this.searchQuery || secret.key.toLowerCase().includes(this.searchQuery.toLowerCase()))
                    : this.secrets;
            },
            secretModalTitle() {
